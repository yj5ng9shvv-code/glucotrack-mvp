...
export function validateHealthSnapshot(payload) {
  ...
  if (!text(profile.fullName, 120) || !text(profile.email, 255) ||
      !integer(profile.age, 0, 130) || !optionalNumber(profile.weightKg, 20, 400) ||
      !optionalNumber(profile.heightCm, 50, 260) || !number(profile.glucoseMmol, 0, 35) ||
      !number(profile.targetGlucoseMmol, 2, 20) || !number(profile.insulinToCarbRatio, 1, 100) ||
      !number(profile.correctionFactor, 0.1, 20)) return "invalid profile values";
