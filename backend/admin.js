...
export function registerAdminRoutes(app, { asyncHandler, services: runtimeServices }) {
  ...
  app.patch("/admin/family/:id/status", adminAuth("users:write"), asyncHandler(adminUpdateFamilyStatus));
...
async function adminFamily(req, res) {
  ...
  if (search) {
    params.push(`%${search}%`);
    where.push("(u.email LIKE $1 OR u.full_name LIKE $1 OR f.invite_email LIKE $1 OR caregiver.email LIKE $1 OR caregiver.full_name LIKE $1)");
  }
  const whereSql = `WHERE ${where.join(" AND ")}`;
  const rows = await pool.query(
    `SELECT COALESCE(f.id, CONCAT('owner-', u.id)) id,
       ...
       COALESCE(caregiver.email, f.invite_email, '') invite_email,
       caregiver.full_name caregiver_name,
       f.member_role role,
       f.permissions,
     ...
     LEFT JOIN users caregiver ON caregiver.id = f.caregiver_user_id
     ${whereSql}
  ...
  const total = await pool.query(
    `SELECT COUNT(*) count FROM users u LEFT JOIN family_links f ON f.owner_user_id = u.id LEFT JOIN users caregiver ON caregiver.id = f.caregiver_user_id ${whereSql}`,
    params
  );
  res.json({ rows: rows.rows, total: Number(total.rows[0]?.count ?? 0), ...page });
}
...
async function adminUpdateFamilyStatus(req, res) {
  ...
  if (!/^\d+$/.test(id) || !["pending", "accepted", "revoked"].includes(status)) {
    return res.status(400).json({ code: "INVALID_FAMILY_STATUS" });
  }
  const result = await pool.query(
  ...
  await writeAudit(req.admin.id, "family_access.status_updated", "family_link", id, req, { status });
