...
function isPlatformToken(platform, token) {
  if (platform === "ios") return APNS_TOKEN_PATTERN.test(token);
  // Direct APNs device tokens must never enter the Android/FCM path.
  return token.length >= 20 && token.length <= 4096 &&
    FCM_TOKEN_PATTERN.test(token) && !APNS_TOKEN_PATTERN.test(token);
}
  ...
  return {
    async register(userId, payload) {
      ...
      if (deviceId.length < 8 || deviceId.length > 128 ||
          !PLATFORM_VALUES.has(platform) || !isPlatformToken(platform, rawToken)) {
