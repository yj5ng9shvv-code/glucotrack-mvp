export function createLocationRepository(query) {
  return {
    ...
    async deleteExpiredLocations(cutoff) {
      return query(
        `DELETE location
         FROM patient_locations AS location
         WHERE location.created_at < $1
           AND NOT EXISTS (
             SELECT 1
             FROM sos_events AS sos
             WHERE sos.patient_id = location.patient_id
    ...
    async deleteExpiredAccessLogs(cutoff) {
      return query(
        "DELETE FROM location_access_logs WHERE created_at < $1",
        [cutoff]
      );
    }
  };
