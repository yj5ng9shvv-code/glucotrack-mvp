...
export function createFamilyRouter({ familyService, memberService, invitationService }) {
  ...
  router.delete("/member/:id", asyncHandler(async (req,res)=>{const family=await familyService.getFamily(req.user.id);await memberService.removeMember(family.id,req.params.id);res.status(204).end();}));
  return router;
}
