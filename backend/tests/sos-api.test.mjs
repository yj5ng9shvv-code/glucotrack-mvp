...
test("repeated SOS create with client event id is idempotent", integrationOptions, async () => {
  await withApi(async ({ request }) => {
    const body = {
    ...
    const first = await request("/api/sos/create", { method: "POST", token: "patient", body });
    const second = await request("/api/sos/create", { method: "POST", token: "patient", body });
    assert.equal(first.status, 201, first.text);
    assert.equal(second.status, 201, second.text);
    assert.equal(second.payload.sos_id, first.payload.sos_id);
    assert.equal(second.payload.client_event_id, "sos-event-202608150001");
  });
});

test("cancel and location updates are idempotent for retry", integrationOptions, async () => {
  await withApi(async ({ request }) => {
    const created = await request("/api/sos/create", { method: "POST", token: "patient" });
    const moved = await request(`/api/sos/location/${created.payload.sos_id}`, {
      method: "POST",
      token: "patient",
      body: { latitude: 52.1, longitude: 21.1, accuracy: 9 }
    });
    assert.equal(moved.status, 200, moved.text);
    assert.equal(moved.payload.latitude, 52.1);

    const cancelled = await request(`/api/sos/cancel/${created.payload.sos_id}`, { method: "POST", token: "patient" });
    const cancelledAgain = await request(`/api/sos/cancel/${created.payload.sos_id}`, { method: "POST", token: "patient" });
    assert.equal(cancelled.status, 200, cancelled.text);
    assert.equal(cancelledAgain.status, 200, cancelledAgain.text);
    assert.equal(cancelledAgain.payload.status, "CANCELLED");
  });
});
...
async function withApi(callback) {
  ...
  let nextId = 1;
  const sosRepository = {
    async createSOS(patientId, latitude, longitude, accuracy, options = {}) {
      const event = {
        id: nextId++, patient_id: Number(patientId), client_event_id: options.clientEventId ?? null,
        status: "ACTIVE", latitude, longitude, accuracy, created_at: new Date().toISOString(),
        cancelled_at: null, resolved_at: null
      };
      events.push(event);
      return { insertId: event.id, rowCount: 1 };
    },
    async findByClientEvent(patientId, clientEventId) {
      return events.find((event) => event.patient_id === Number(patientId) && event.client_event_id === clientEventId) ?? null;
    },
    ...
    async updateLocation(id, patientId, latitude, longitude, accuracy) {
      const event = events.find((item) => item.id === Number(id) && item.patient_id === Number(patientId) && item.status === "ACTIVE");
      if (!event) return { rowCount: 0 };
