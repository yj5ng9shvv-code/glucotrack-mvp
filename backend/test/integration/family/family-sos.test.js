...
test('SOS creation validates the payload and rejects requests without a token', async () => {
  const api = testApi();
  const unauthorized = await api.request('/sos/events', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ latitude: 52.2297, longitude: 21.0122 }),
  });
  assert.equal(unauthorized.status, 401);

  const patientToken = await login(api, 'patient@test.com');
  const invalid = await api.request('/sos/events', {
    method: 'POST',
    headers: { ...auth(patientToken), 'Content-Type': 'application/json' },
    body: JSON.stringify({ latitude: 91, longitude: 21.0122 }),
  });
  assert.equal(invalid.status, 400);

  const valid = await api.request('/sos/events', {
    method: 'POST',
    headers: { ...auth(patientToken), 'Content-Type': 'application/json' },
    body: JSON.stringify({ latitude: 52.2297, longitude: 21.0122, accuracyMeters: 8, glucoseMmol: 3.2 }),
  });
