...
test('Family invitations are email-bound, expire, and cannot be reused', async () => {
  ...
  const [[dispatched]] = await withTestDatabase((db) => db.execute(
    'SELECT invite_code, invite_code_hash, email_sent, email_error FROM family_links WHERE id=?',
    [mismatch.id]
  ));
  assert.equal(dispatched.invite_code, null);
  assert.match(String(dispatched.invite_code_hash), /^[a-f0-9]{64}$/);
  assert.equal(Number(dispatched.email_sent), 1);
  assert.equal(dispatched.email_error, null);
