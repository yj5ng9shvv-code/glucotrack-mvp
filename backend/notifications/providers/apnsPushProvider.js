...
export async function createApnsPushProvider({
  ...
  return {
    ...
    async send({ token, title, body, data }) {
      ...
      notification.threadId = normalizedData.apns_thread_id || "glukotrack-sos";
      notification.collapseId = normalizedData.apns_collapse_id || normalizedData.idempotency_key || normalizedData.event_id;
      notification.payload = normalizedData;
      try {
