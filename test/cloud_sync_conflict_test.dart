...
void main() {
  test('retries a conflicting push with merged diary entries and new revision', () async {
    SharedPreferences.setMockInitialValues({'cloudSyncRevision': 2});
    final state = AppState(authService: _OfflineAuthService());
    await state.load();
    await state.applyServerSnapshot(_snapshot(
      'phone-entry',
      sensorReadings: [
        {
    ...
    var requests = 0;
    final service = CloudSyncService(client: MockClient((request) async {
      requests += 1;
        ...
        return http.Response(
          jsonEncode({
            ...
            'payload': _snapshot(
              ...
              sensorReadings: [
                {
                  'time': '', // invalid sensor reading should not be merged as duplicate
                  'glucoseMmol': 6.5,
                  'brand': '',
                  'trend': 'steady',
                  'sourceId': '',
...
Map<String, dynamic> _snapshot(String id, {List<Map<String, dynamic>>? sensorReadings}) => {
      'profile': {
        'fullName': 'Sync User', 'email': 'sync@example.com', 'phone': '',
        'age': 40, 'weightKg': 75.0, 'heightCm': 180.0, 'languageCode': 'en',
        'glucoseUnitPreference': 'mmolL', 'diabetesType': 'type1',
        'targetGlucoseMmol': 6.0, 'insulinToCarbRatio': 10.0,
        'correctionFactor': 2.0, 'glucoseMmol': 6.0,
      },
      'diaryEntries': [
        {
          'id': id, 'time': '2026-07-11T10:00:00Z', 'type': 'meal',
          'glucoseMmol': 6.0, 'carbs': 10.0, 'insulinUnits': 1.0,
          'title': 'Meal', 'note': '', 'source': 'manual',
        }
      ],
