...
void main() {
  ...
  test('deleted diary entries stay deleted after a stale pull and conflict',
      () async {
    SharedPreferences.setMockInitialValues({'cloudSyncRevision': 2});
    final state = AppState(authService: _OfflineAuthService());
    await state.load();
    await state.useDeviceManagementToken('token');
    await state.applyServerSnapshot(_snapshot('deleted-entry'));
    await state.removeDiaryEntry('deleted-entry');

    await state.applyServerSnapshot(_snapshot('deleted-entry'));
    expect(state.diaryEntries, isEmpty);
        ...
        if (requests == 1) {
          return http.Response(
            jsonEncode({
              'code': 'SYNC_CONFLICT',
              'revision': 3,
              'payload': _snapshot('deleted-entry'),
            }),
            409,
          );
        }
        final payload =
            jsonDecode(request.body)['payload'] as Map<String, dynamic>;
        expect(payload['diaryEntries'], isEmpty);
        expect(payload['deletedDiaryEntryIds'], contains('deleted-entry'));
        return http.Response(jsonEncode({'ok': true, 'revision': 4}), 200);
      }),
    );

    expect((await service.push(state)).ok, isTrue);
    expect(requests, 2);
  });

  test('pull unwraps stored request envelope and restores glucose units',
      () async {
    ...
    final result = await service.pull(state);

    expect(result.ok, isTrue);
    expect(state.glucoseUnitPreference, GlucoseUnitPreference.mgDl);
  });

  test('pull preserves local diary entries before the next push', () async {
    SharedPreferences.setMockInitialValues({});
    final state = AppState(authService: _OfflineAuthService());
    await state.load();
    await state.useDeviceManagementToken('token');
    await state.addDiaryEntry(
      DiaryLogEntry(
        id: 'local-entry',
        time: DateTime.parse('2026-07-11T11:00:00Z'),
        type: DiaryLogType.note,
        glucoseMmol: 6.4,
        carbs: 0,
        insulinUnits: 0,
        title: 'Local note',
        note: 'Unsynced local diary entry',
      ...
      client: MockClient((request) async {
        ...
        return http.Response(
          jsonEncode({
            ...
            'snapshot': {
              'revision': 7,
              'payload': _snapshot('server-entry'),
            },
          }),
          200,
          headers: {'content-type': 'application/json'},
    ...
    expect(result.ok, isTrue);
    expect(state.diaryEntries.map((entry) => entry.id).toSet(), {
      'local-entry',
      'server-entry',
    });
    final preferences = await SharedPreferences.getInstance();
    expect(preferences.getInt('cloudSyncRevision'), 7);
  });
}
...
Map<String, dynamic> _snapshot(
  ...
  String glucoseUnitPreference = 'mmolL',
}) =>
    {
      'profile': {
        'fullName': 'Sync User',
        ...
        'heightCm': 180.0,
        'languageCode': 'en',
        'glucoseUnitPreference': glucoseUnitPreference,
        'diabetesType': 'type1',
