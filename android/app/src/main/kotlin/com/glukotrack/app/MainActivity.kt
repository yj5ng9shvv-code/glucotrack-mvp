...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
            ...
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    ...
                    "cancelEmergencyAlert" -> {
                        NotificationManagerCompat.from(this).cancel(4102)
                        result.success(null)
                    }
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "glucotrack/live_location")
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "start" -> {
                        ...
                        val baseUrl = call.argument<String>("apiBaseUrl").orEmpty().trimEnd('/')
                        val intent = Intent(this, LiveLocationService::class.java).apply {
                            action = LiveLocationService.ACTION_START
                            putExtra("token", token)
                            putExtra("endpoint", "$baseUrl/family/live-location/position")
                        }
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) startForegroundService(intent) else startService(intent)
                        result.success(null)
                    }
                    "stop" -> {
                        startService(Intent(this, LiveLocationService::class.java).apply {
                    ...
                    "startSos" -> {
                        ...
                        val token = call.argument<String>("token").orEmpty()
                        val baseUrl = call.argument<String>("apiBaseUrl").orEmpty().trimEnd('/')
                        val eventId = call.argument<String>("eventId").orEmpty()
                        val intent = Intent(this, LiveLocationService::class.java).apply {
                            action = LiveLocationService.ACTION_START
                            putExtra("token", token)
                            putExtra("endpoint", "$baseUrl/sos/events/$eventId/location")
                            putExtra("sosMode", true)
                        }
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) startForegroundService(intent) else startService(intent)
                        result.success(null)
                    }
