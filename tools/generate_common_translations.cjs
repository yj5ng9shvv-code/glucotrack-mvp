...
const entries = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'reports', 'common-fallback-source.json'), 'utf8'));
...
async function main() {
  ...
  const output = ['const Map<String, Map<String, String>> commonTranslations = {'];
  ...
  output.push('};', '');
  const target = path.join(process.cwd(), 'lib', 'l10n', 'common_translations.dart');
